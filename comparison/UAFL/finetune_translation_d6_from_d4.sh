...
# Curriculum UAFL non-registration fine-tuning: strongest radial d=4 -> radial d=6.
# Train patch remains 64x64; only HR-MSI is warped.
# d=6 means sqrt(dx^2+dy^2) <= 6 px:
#   r~U(0,6), theta~U(0,2pi), dx=r*cos(theta), dy=r*sin(theta).
#
# The launcher scans all non-context UAFL best checkpoints, keeps only
# PaviaU/physical/translation/d=4 checkpoints, selects the one with the largest
# stored best_psnr, preserves model + AdamW + epoch, resets only best_psnr for
# the harder d=6 validation distribution, and trains 240 more epochs.

D6_DIR="comparison/UAFL/checkpoints/physical_translation_d6_ft_strongest_d4/PaviaU"
D6_LOG="comparison/UAFL/logs/physical_translation_d6_ft_strongest_d4/PaviaU"
D6_INIT="${D6_DIR}/resume_strongest_d4_reset_best.pth.tar"
META_FILE="${D6_DIR}/selected_d4_meta.txt"

mkdir -p "${D6_DIR}" "${D6_LOG}"

python - "${D6_INIT}" "${META_FILE}" <<'PY'
...
for path in root.rglob("best.pth.tar"):
    ...
    try:
        ...
        continue
    if not math.isfinite(d) or abs(d - 4.0) > 1e-8:
        continue
...
if not candidates:
    raise FileNotFoundError(
        "No non-context UAFL PaviaU physical translation d=4 best checkpoint found under "
...
print("Eligible d=4 best checkpoints:")
for psnr, epoch, path, _ in candidates:
    print(f"  PSNR={psnr:.6f} epoch={epoch} path={path}")
print(
    f"Selected strongest d=4 source: PSNR={best_psnr:.6f}, "
...
print(f"Prepared d=6 resume -> {out_ckpt}; target_epoch={target_epoch}")
PY
