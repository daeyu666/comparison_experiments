...
from model_ssm_fuse9_2 import Net
...
parser.add_argument('--data', default="/media/xd132/USER/XY/AAAI/AAAI/Dataset/PaviaC", metavar='DIR')
parser.add_argument('--type', default='', choices=['', '_alpha1', '_alpha2', '_alpha3'])
parser.add_argument('--workers', default=16, type=int)
parser.add_argument('--epochs', default=3001, type=int)
...
def train(train_loader, model, loss_l1, loss_l2, loss_NCC, optimizer, epoch, args, tf_writer):
    ...
    #     '/media/xd132/USER/ZTZ/wy_fuse/base/test/log/pavia_batchsize_8_epochs_3001_lr_0.001_reg_MS_/MDF_NREG_2700.pth.tar')[
    #                           'state_dict'])
    ...
    for step, (lrhs, pan, gths) in enumerate(train_loader):
        ...
        # # loss2_1 = loss_l1(reg_loss(I_M), reg_loss(PAN_M_reged))
        # # loss2_2 = loss_l1(reg_loss(I_H.detach()), reg_loss(PAN_H_reged))
        ...
        # # loss2_1 = cross_correlation_loss(UseDSP(I_M, DSPM1), PAN_M, n=3)
        # # loss2_2 = cross_correlation_loss(UseDSP(I_H, DSPH1), PAN_H, n=3)
        # loss2 = (loss2_1 + loss2_2 + loss2_3 + loss2_4) / 4
        # loss2 = loss2_1

        # 平滑损失
        loss3_1 = smooothing_loss(RG_1)
        loss3_2 = smooothing_loss(RG_2)
        loss3 = (loss3_1 + loss3_2)/2

        loss = loss1 + 0.01*loss3 + 0.1*loss2
        ...
        if step % args.print_freq == 0:
            output = ('Epoch: [{0}][{1}/{2}] \t'
                ...
                loss=loss_all, l1_loss=l1_loss_all, smo_loss=smo_loss_all))
            print(output)
    # print(loss2_1)
    # print(loss2_2)
    ...
    # tf_writer.add_image("reg_I", RG_1[0,:,:,:].cpu().detach().numpy(), epoch, dataformats='CHW')
...
# def train(root, type):
#     # 数据准备
#     data = MyDataset(root, "train", type)
#     data_loader = DataLoader(data, batch_size=batchsize, shuffle=True, drop_last=True)
#     best_loss = 500
#     loss_epoch = 0
#     loss_mat = []
#
#     # 模型加载
#     model = Main(80, 102, batchsize)
#     model = model.to(device)
#     model.train()
#     optimization = optim.Adam(model.parameters(), lr=learningrate)
#     scheduler = optim.lr_scheduler.StepLR(optimization, step_size=20, gamma=0.8)
#     loss_l1 = nn.L1Loss()
#     loss_l2 = nn.MSELoss()
#
#     # 模型训练
...
#
#             HRHS, HS_M_loss, HS_H_loss, PAN_M, PAN_H, DSPM1, DSPM2, DSPM3, DSPH1, DSPH2, DSPH3 = model(pan, lrhs)
#             """LOSS区域"""
#             loss1 = loss_l1(HRHS, gths)
#             # 配准损失
#             loss2_1 = loss_l1(reg_loss(PAN_M), reg_loss(UseDSP(HS_M_loss, DSPM1)))
#             loss2_2 = loss_l1(reg_loss(PAN_M), reg_loss(UseDSP(HS_M_loss, DSPM2)))
#             loss2_3 = loss_l1(reg_loss(PAN_M), reg_loss(UseDSP(HS_M_loss, DSPM3)))
#             loss2_4 = loss_l1(reg_loss(PAN_H), reg_loss(UseDSP(HS_H_loss, DSPH1)))
#             loss2_5 = loss_l1(reg_loss(PAN_H), reg_loss(UseDSP(HS_H_loss, DSPH2)))
#             loss2_6 = loss_l1(reg_loss(PAN_H), reg_loss(UseDSP(HS_H_loss, DSPH3)))
#             loss2 = loss2_1 + loss2_2 + loss2_3 + loss2_4 + loss2_5 + loss2_6
#
#             loss = 3*loss1/loss1.detach() #+ loss2/loss2.detach()
#             losslog = loss1 #+ loss2
#             optimization.zero_grad()
#             loss.backward()
#             optimization.step()
#             loss_mat.append(losslog.detach().cpu())
#
#             loss_epoch = loss_epoch + losslog
#
#         loss_epoch = loss_epoch/(step+1)
#
#         if loss_epoch <= best_loss:
#             best_loss = loss_epoch
#             torch.save(model.state_dict(), './dataout/pavia' + type + '/best.pkl')
#         # scheduler.step()
#         # print('epoch:' + str(epoch), '--best_loss:' + str(best_loss.item()), '--loss_epoch:' + str(loss_epoch.item()),
#         #     '--loss:' + str(losslog.item()), 'loss1' + str(loss1.item()))
#         print('epoch: {:d} --best_loss: {:.3f} --loss_epoch: {:.3f} --loss: {:.3f} --loss1: {:.5f}'\
#               .format(epoch, best_loss, loss_epoch, losslog, loss1))
#         loss_epoch = 0
#
#
#     torch.save(model.state_dict(), './dataout/pavia' + type + '/last.pkl')
#     savemat('./dataout/pavia' + type + '/loss.mat', {'loss': loss_mat})
class NCC:
    """
...
if __name__ == "__main__":
    ...
    # model = Main(size_E=80, bands=102, args=args).cuda()
    #
    # # 输入模型的尺寸，这个尺寸应该与模型输入的尺寸相匹配
    # input1 = torch.randn(1, 4, 160, 160).cuda()
    # input2 = torch.randn(1, 102, 40, 40).cuda()
    #
    # # 使用thop分析模型的运算量和参数量
