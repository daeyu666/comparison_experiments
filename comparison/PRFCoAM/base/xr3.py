...
import numbers
from mamba_simple_xr3 import Mamba
...
class Net(nn.Module):
    ...
    def forward(self, LRHS, HRMS):
        ...
        LRHS_FUSE_2 = self.FUSE_2(LRHS_LRMS_RG_1, HRMS_LRMS_2)
        LRHS_LRMS_RG_2, RG_2 = self.RG(self.R(LRHS_FUSE_2), HRMS_LRMS_2, LRHS_FUSE_2)

        HRHS = self.FUSE_4(LRHS_LRMS_RG_2, HRMS)
        HRHS = self.fuse_last(HRHS)

        HRHS_HRMS = self.R(HRHS)
        HRHS_LRHS_2 = self.BP(HRHS_HRMS)

        return HRHS, HRHS_HRMS, HRHS_LRHS_2, LRHS_LRMS_RG_2, RG_1, RG_2
