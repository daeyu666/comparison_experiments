...
class ChannelAttentionModule(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool1d(1)  # (1) 表示输出的高度和宽度都被设置为 1。
...
class Mamba(nn.Module):
    def __init__(
        ...
        self.ca = ChannelAttentionModule(d_model)
        ...
        if bimamba_type == "v1":
            ...
            self.in_proj_extra1 = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
            self.in_proj_extra2 = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
    ...
    def forward(self, hidden_states, inference_params=None, extra_emb1=None, extra_emb2=None):
        ...
        if extra_emb1 is None and extra_emb2 is None:
            ...
            extra_emb1 = torch.cat([extra_emb1, extra_emb1], 1)
            extra_emb2 = rearrange(
                self.in_proj_extra2.weight @ rearrange(extra_emb2, "b l d -> d (b l)"),
                "d (b l) -> b d l",
                l=seqlen,
            )
            extra_emb2 = torch.cat([extra_emb2, extra_emb2], 1)
            if self.in_proj_extra1.bias is not None:
                extra_emb1 = extra_emb1 + rearrange(self.in_proj_extra1.bias.to(dtype=xz.dtype), "d -> d 1")
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                # F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
                if not self.if_devide_out:
                    out = F.linear(rearrange(out + out_b + out_c, "b d l -> b l d"), self.out_proj.weight,
                                   self.out_proj.bias)
                else:
                    out = F.linear(rearrange(out + out_b + out_c, "b d l -> b l d") / 3, self.out_proj.weight,
                                   self.out_proj.bias)
            else:
                out = mamba_inner_fn(
                    xz,
...
x = torch.randn(2, 40*40, 31).to(device)
net = Mamba(31).to(device)
b = net(x)
print(b.shape)