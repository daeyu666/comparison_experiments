...
class B(nn.Module):
    def __init__(self, sin):
        ...
        self.conv1 = nn.Conv2d(sin, sin, 3, 2, 1)
...
class Net(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.FUSE = FUSE_Net(cin, cout, 2*2)
    ...
    def forward(self, LRHS, HRMS):
        ...
        LRHS_LRMS_RG, RG = self.RG(LRHS_LRMS, HRMS_LRMS_2, LRHS)

        HRHS = self.FUSE(LRHS_LRMS_RG, HRMS)
        HRHS = self.fuse_last(HRHS)

        HRHS_HRMS = self.R(HRHS)
        HRHS_LRHS_2 = self.BP(HRHS_HRMS)

        return HRHS, HRHS_HRMS, HRHS_LRHS_2, LRHS_LRMS_RG, RG
