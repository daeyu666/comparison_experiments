...
try:
    from mamba_simple_xr3.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, bimamba_inner_fn, \
...
try:
    from mamba_simple_xr3.ops.triton.selective_state_update import selective_state_update
except ImportError:
    selective_state_update = None

try:
    from mamba_simple_xr3.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None, extra_emb1=None, extra_emb2=None):
        ...
        if self.bimamba_type == "v2":
            ...
            # print('1', xz.shape)
            xz = torch.cat([hidden_states, hidden_states], 1)
            xz = xz.flatten(2, 3)
            # print('2', xz.shape)
            A_b = -torch.exp(self.A_b_log.float())
