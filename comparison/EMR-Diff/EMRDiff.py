...
class Edge(nn.Module):
    def __init__(self):
        ...
        sobel_kernel_x = th.tensor(
            [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=th.float32
        )
        sobel_kernel_y = th.tensor(
            [[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=th.float32
        )
        self.sobel_x.weight.data.copy_(sobel_kernel_x.view(1, 1, 3, 3))
        self.sobel_y.weight.data.copy_(sobel_kernel_y.view(1, 1, 3, 3))
    ...
    def compute_edge_map(self, rgb):
        ...
        edge_map = (edge_magnitude - min_val) / (max_val - min_val + 1e-8)
        return edge_map + 1.0

    def forward(self, rgb):
        return self.compute_edge_map(rgb)
...
class EdgeAwareSpectralResidual(nn.Module):
    ...
    def compute_edge_map(self, rgb):
        ...
        min_val = edge_magnitude.view(b, -1).min(dim=1)[0].view(b, 1, 1, 1)
        max_val = edge_magnitude.view(b, -1).max(dim=1)[0].view(b, 1, 1, 1)
        return (edge_magnitude - min_val) / (max_val - min_val + 1e-8)
...
class EMRDIFF:
    def __init__(self, configs):
        ...
        band_dim = int(opt.get('band_dim', 0))

        if self.schedule_name == 'exponential':
            etas_start = min(min_noise_level / self.kappa, min_noise_level)
            increaser = math.exp(
                1 / (num_diffusion_timesteps - 1)
                * math.log(etas_end / etas_start)
            )
        ...
        elif self.schedule_name in ('olss', 'uniform'):
            etas_start = min(min_noise_level / self.kappa, min_noise_level)
            self.sqrt_etas = np.linspace(
                etas_start, etas_end, num_diffusion_timesteps, dtype=np.float64
            )
            self.etas = self.sqrt_etas ** 2
        else:
    ...
    def to(self, device):
        self.residual_calculator = self.residual_calculator.to(device)
        return self
    ...
    def forward_addnoise(self, x_start, y, t, noise, rgb_hr=None):
        ...
        return (
            _extract_into_tensor(self.etas, t, x_start.shape) * resy
            + x_start
            + _extract_into_tensor(
                self.sqrt_etas * self.kappa, t, x_start.shape
            ) * modulated_noise
        )
    ...
    def prior_sample(self, y, noise, edge_map=None):
        if edge_map is None:
            edge_map = th.ones_like(noise[:, :1])
        t = th.tensor(
            [self.num_diffusion_timesteps - 1] * y.shape[0],
            device=y.device,
        ).long()
        weighted_noise = noise * edge_map
        return y + _extract_into_tensor(
            self.kappa * self.sqrt_etas, t, y.shape
        ) * weighted_noise
